export default function collectLocation () {
  ...
  Object.keys(allStyle).forEach(key => {
    auchorPoints = findLineStyle(allStyle[key].rules)
...
function parseLocation (anchorArr) {
  ...
  // 坐标的映射 对称
  const anchorPoints = [half]

  for (let i = 0; i < 3; i++) {
    const [, a, , b] = anchorArr[i]
    const anchorPoint = (+a) + (+b)

    anchorPoints.push(anchorPoint + half)
  }

  anchorPoints.push(+cellWidth + (+anchor[3]), half)
  return anchorPoints
}